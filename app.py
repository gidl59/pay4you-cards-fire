...
app = Flask(__name__)
# QUESTA È LA RIGA CHE MANCAVA E CHE RISOLVE L'ERRORE 500:
app.jinja_env.add_extension('jinja2.ext.do')

app.secret_key = "pay4you_2026_goodnight_fix"
...
def reset_db_emergency():
    if os.path.exists(DB_FILE):
        try:
            os.remove(DB_FILE)
            return "DB CANCELLATO. Vai su /master"
        except: return "Errore cancellazione."
    return "DB PULITO. Vai su /master"
...
def master_add():
    clienti = load_db()
    if not clienti: new_id = 1
    else: new_id = max([c['id'] for c in clienti]) + 1
    ...
    new_c = {
        "id": new_id, "username": request.form.get('username') or f"user{new_id}",
        "password": final_pass, "slug": slug, "nome": request.form.get('nome') or "Nuovo",
        "azienda": "New", "p1": {"active": True}, "p2": {"active": False}, "p3": {"active": False}
    }
    repair_user(new_c)

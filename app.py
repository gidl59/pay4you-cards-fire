...
app = Flask(__name__)
app.secret_key = "pay4you_2026_final_syntax_fix"
...
# --- GESTIONE DB (LA PARTE CORRETTA) ---
def load_db():
    # Se il file non esiste, ritorna lista vuota
    if not os.path.exists(DB_FILE):
        return []
    
    # SINTASSI CORRETTA: 'try' e 'with' su righe diverse
    try:
...
def repair_user(user):
    ...
    for pid in ['p1', 'p2', 'p3']:
        ...
        defaults = {
            ...
            'piva':'', 'cod_sdi':'', 'pec':'',
            'fx_rotate_logo':'off', 'fx_rotate_agent':'off', 'fx_interaction':'tap', 'fx_back_content':'logo',
            'pos_x':50, 'pos_y':50, 'zoom':1, 'trans':{'en':{},'fr':{},'es':{},'de':{}}
        }
...
def deactivate_profile(p_id):
    if p_id=='1': return "No P1"
    clienti=load_db(); user=next((c for c in clienti if c['id']==session.get('user_id')), None)
    user['p'+p_id]['active']=False; save_db(clienti)
    return redirect(url_for('area'))
...
def edit_profile(p_id):
    ...
    # SE L'UTENTE NON ESISTE (es. DB cancellato), LOGOUT IMMEDIATO
    if not user:
        session.clear()
    ...
    if request.method == 'POST':
        ...
        p['trans']={'en':{'role':request.form.get('role_en'),'bio':request.form.get('bio_en')},'fr':{'role':request.form.get('role_fr'),'bio':request.form.get('bio_fr')},'es':{'role':request.form.get('role_es'),'bio':request.form.get('bio_es')},'de':{'role':request.form.get('role_de'),'bio':request.form.get('bio_de')}}
...
def master_add():
    ...
    final_pass=request.form.get('password') or auto_pass; slug=request.form.get('slug') or f"card-{new_id}"
    new_c={"id":new_id, "username":request.form.get('username') or f"user{new_id}", "password":final_pass, "slug":slug, "nome":request.form.get('nome') or "Nuovo", "azienda":"New", "p1":{"active":True},"p2":{"active":False},"p3":{"active":False}}
...
def view_card(slug):
    ...
    p_req=request.args.get('p','p1'); p=user[p_req] if user.get(p_req,{}).get('active') else user['p1']
    ag={"name":p.get('name'),"role":p.get('role'),"company":p.get('company'),"bio":p.get('bio'),"photo_url":p.get('foto'),"logo_url":p.get('logo'),"personal_url":p.get('personal_foto'),"slug":slug,"piva":p.get('piva'),"pec":p.get('pec'),"cod_sdi":p.get('cod_sdi'),"fx_rotate_logo":p.get('fx_rotate_logo'),"fx_rotate_agent":p.get('fx_rotate_agent'),"fx_interaction":p.get('fx_interaction'),"fx_back":p.get('fx_back_content'),"photo_pos_x":p.get('pos_x',50),"photo_pos_y":p.get('pos_y',50),"photo_zoom":p.get('zoom',1),"trans":p.get('trans',{})}

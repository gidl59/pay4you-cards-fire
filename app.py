...
def parse_pdf_items(pdf1_url: str, limit: int = MAX_PDFS):
    items = []
    if not pdf1_url:
        return items
...
def compact_pdfs_keep10(pdf1_url: str) -> str:
    # legge tutto, dedup per url, max 10
    all_items = parse_pdf_items(pdf1_url or "", limit=9999)
    seen = set()
    out = []
    for it in all_items:
...
def _set_profile_json(form: dict) -> dict:
    data = {}
    for k in [
...
def set_profile_data_p2(agent: Agent, form: dict):
    agent.p2_json = json.dumps(_set_profile_json(form), ensure_ascii=False)
    agent.updated_at = dt.datetime.utcnow()

def set_profile_data_p3(agent: Agent, form: dict):
    agent.p3_json = json.dumps(_set_profile_json(form), ensure_ascii=False)
    agent.updated_at = dt.datetime.utcnow()
...
def handle_media_uploads(ag: Agent):
    ...
    if gallery_files:
        ...
        for f in gallery_files:
            if len(urls) >= MAX_GALLERY_IMAGES:
                break
            urls.append(save_upload(f, "images"))
    ...
    if video_files:
        ...
        for f in video_files:
            if len(urls) >= MAX_VIDEOS:
                break
            urls.append(save_upload(f, "videos"))
        urls = [u for u in urls if u]
        existing = [x for x in (ag.video_urls or "").split("|") if x.strip()]
        combined = (existing + urls)[:MAX_VIDEOS]
        ag.video_urls = "|".join(combined)

    # pdf (sempre max 10, no duplicati)
    ag.pdf1_url = compact_pdfs_keep10(ag.pdf1_url or "")

    existing_pdf = parse_pdf_items(ag.pdf1_url or "", limit=9999)
    ...
    # ricompone max 10 + dedup
    tmp = []
    seen = set()
    for item in out:
        u = (item.get("url") or "").strip()
        n = (item.get("name") or "Documento").strip()
        if not u or u in seen:
            continue
        seen.add(u)
        tmp.append(f"{n}||{u}")
...
def edit_agent(slug):
    ...
    pdfs = parse_pdf_items(compact_pdfs_keep10(ag.pdf1_url or ""), limit=MAX_PDFS)

    # salva eventuale compattazione
    if ag.pdf1_url != compact_pdfs_keep10(ag.pdf1_url or ""):
        ag.pdf1_url = compact_pdfs_keep10(ag.pdf1_url or "")
        s.commit()
...
def admin_profile3(slug):
    ...
    if (ag.p3_enabled or 0) != 1:
        flash("Profilo 3 non attivo", "error")
        return redirect(url_for("edit_agent", slug=slug))

    if request.method == "POST":
        set_profile_data_p3(ag, request.form)
        s.commit()
...
# ==========================
# ADMIN: activate / deactivate P2/P3 (sempre vuoti)
# ==========================
...
def me_deactivate_p2():
    ...
    ag.updated_at = dt.datetime.utcnow()
    s.commit()
    flash("Profilo 2 disattivato (svuotato).", "ok")
    return redirect(url_for("dashboard"))
...
def me_activate_p3():
    ...
    ag.updated_at = dt.datetime.utcnow()
    s.commit()
    flash("Profilo 3 attivato (vuoto).", "ok")
    return redirect(url_for("dashboard"))
...
def me_deactivate_p3():
    ...
    ag.updated_at = dt.datetime.utcnow()
    s.commit()
    flash("Profilo 3 disattivato (svuotato).", "ok")
    return redirect(url_for("dashboard"))
...
def qr_png(slug):
    ...
    # ✅ INLINE (non attachment) così si apre al click (anche da WhatsApp)
...
# ==========================
# MEDIA DELETE (robusto + pdf dedup + cap 10)
# ==========================
...
def delete_media(slug):
    ...
    if t == "gallery":
        ...
        # legge tutto, elimina davvero, poi dedup + max10
        items = parse_pdf_items(ag.pdf1_url or "", limit=9999)
        if 0 <= idx < len(items):
            items.pop(idx)
        ...
        for it in items:
            ...
            if len(cleaned) >= MAX_PDFS:
                break

        ag.pdf1_url = "|".join([f"{x['name']}||{x['url']}" for x in cleaned])
    ...
    # torna dove stavi
    if is_admin():
        # se sto modificando P2/P3 torno comunque all'edit P1 (ok per te)
        return redirect(url_for("edit_agent", slug=slug))
    return redirect(url_for("me_edit"))
...
def card(slug):
    ...
    p3_enabled = int(ag.p3_enabled or 0) == 1

    use_p2 = (p_key == "p2" and p2_enabled)
    use_p3 = (p_key == "p3" and p3_enabled)

    data = {}
    if use_p2:
        data = load_p2_data(ag)
    elif use_p3:
        data = load_p3_data(ag)

    def pick(key: str) -> str:
        if use_p2 or use_p3:
            return (data.get(key) or "").strip()
        return (getattr(ag, key) or "").strip()

    emails = split_csv(pick("emails"))
    websites = split_csv(pick("websites"))

    addresses_raw = pick("addresses")
    addresses = split_lines(addresses_raw)
    addr_objs = []
    for a in addresses:
        q = a.replace(" ", "+")
    ...
    mobiles = []
    m1 = pick("phone_mobile")
    m2 = pick("phone_mobile2")
    if m1:
        mobiles.append(m1)
    if m2:
        mobiles.append(m2)

    office_value = pick("phone_office")

    gallery = [x for x in (ag.gallery_urls or "").split("|") if x.strip()]
    videos = [x for x in (ag.video_urls or "").split("|") if x.strip()]
    pdfs = parse_pdf_items(compact_pdfs_keep10(ag.pdf1_url or ""), limit=MAX_PDFS)

    wa_link = pick("whatsapp")
    ...
    if use_p3:
        qr_url = f"{base_url}/{ag.slug}?p=p3"

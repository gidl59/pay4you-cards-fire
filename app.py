...
def parse_profiles_json(raw: str):
    ...
    for i, p in enumerate(data):
        ...
        out.append({
            ...
            # ✅ contenuti EN (optional)
            "name_en": (p.get("name_en") or "").strip(),
            "role_en": (p.get("role_en") or "").strip(),
            "company_en": (p.get("company_en") or "").strip(),
            "bio_en": (p.get("bio_en") or "").strip(),
        })
...
def agent_to_view(ag: Agent):
    return SimpleNamespace(
        ...
        name_en=getattr(ag, "name_en", "") or "",
        company_en=getattr(ag, "company_en", "") or "",
        role_en=getattr(ag, "role_en", "") or "",
        bio_en=getattr(ag, "bio_en", "") or "",
...
def apply_profile_to_view(view, profile: dict, lang: str):
    """Applica il profilo scelto al view (IT o EN)."""
    if not profile:
        return view

    # foto/logo sempre uguali (non dipendono dalla lingua)
...
def apply_lang_overrides_on_agent(view, ag: Agent, lang: str):
    """Per profilo principale: se lang=en e campo EN pieno, sovrascrive."""
    if lang != "en":
        return view

    if getattr(ag, "name_en", None):
        view.name = ag.name_en
    if getattr(ag, "role_en", None):
        view.role = ag.role_en
    if getattr(ag, "company_en", None):
        view.company = ag.company_en
    if getattr(ag, "bio_en", None):
        view.bio = ag.bio_en
...
def create_agent():
    ...
    fields = [
        ...
        "name_en","company_en","role_en","bio_en",
...
# ✅ PROFILO 2 SEMPLICE (salva p2 in profiles_json) con IT/EN
@app.get("/me/profile2")
@login_required
def me_profile2():
    if is_admin():
...
def me_profile2_post():
    ...
    company_en = (request.form.get("company_en") or "").strip()
    bio_en = (request.form.get("bio_en") or "").strip()

    if not name:
        profiles = parse_profiles_json(getattr(ag, "profiles_json", "") or "")
        p2 = select_profile(profiles, "p2") or {"key":"p2"}
        return render_template("profile2_form.html", agent=ag, p2=p2, error="Il nome (IT) è obbligatorio.", ok=None)

    photo = request.files.get("photo")
    logo = request.files.get("logo")

    photo_url = upload_file(photo, "photos") if photo and photo.filename else None
    logo_url = upload_file(logo, "logos") if logo and logo.filename else None
...
def public_card(slug):
    ...
    # 1) se hai un profilo attivo, applica IT/EN dal profilo
    if active_profile:
        ag_view = apply_profile_to_view(ag_view, active_profile, lang)
    else:
        # 2) altrimenti usa i campi EN del profilo principale (se esistono)
        ag_view = apply_lang_overrides_on_agent(ag_view, ag, lang)
    ...
    # label profili in lingua
    profiles_for_ui = []
    for p in profiles:
        p2 = dict(p)
        if lang == "en" and p2.get("label_en"):
            p2["label_ui"] = p2["label_en"]
        else:
            p2["label_ui"] = p2.get("label") or p2.get("key")
        profiles_for_ui.append(p2)
